POPULAR_TAGS = [
    # ── Genshin Impact ────────────────────────────────────────────────────────
    # Mondstadt
    "amber_(genshin_impact)",
    "barbara_(genshin_impact)",
    "fischl_(genshin_impact)",
    "jean_(genshin_impact)",
    "kaeya_(genshin_impact)",       # keep for fuzzy-match coverage
    "lisa_(genshin_impact)",
    "noelle_(genshin_impact)",
    "sucrose_(genshin_impact)",
    "venti_(genshin_impact)",
    "mona_(genshin_impact) ",

    # Liyue
    "beidou_(genshin_impact)",
    "ganyu_(genshin_impact)",
    "hu_tao_(genshin_impact)",
    "keqing_(genshin_impact)",
    "ningguang_(genshin_impact)",
    "qiqi_(genshin_impact)",
    "shenhe_(genshin_impact)",
    "xiangling_(genshin_impact)",
    "xingqiu_(genshin_impact)",
    "xinyan_(genshin_impact)",
    "yanfei_(genshin_impact)",
    "yelan_(genshin_impact)",
    "yun_jin_(genshin_impact)",

    # Inazuma
    "gorou_(genshin_impact)",
    "kamisato_ayaka",
    "kamisato_ayato",
    "kirara_(genshin_impact)",
    "kujou_sara",
    "kuki_shinobu",
    "raiden_shogun",
    "sangonomiya_kokomi",
    "sayu_(genshin_impact)",
    "thoma_(genshin_impact)",
    "yae_miko",
    "yoimiya_(genshin_impact)",

    # Sumeru
    "al-haitham_(genshin_impact)",
    "candace_(genshin_impact)",
    "collei_(genshin_impact)",
    "cyno_(genshin_impact)",
    "dehya_(genshin_impact)",
    "dori_(genshin_impact)",
    "faruzan_(genshin_impact)",
    "layla_(genshin_impact)",
    "nahida_(genshin_impact)",
    "nilou_(genshin_impact)",
    "tighnari_(genshin_impact)",

    # Fontaine
    "arlecchino_(genshin_impact)",
    "charlotte_(genshin_impact)",
    "chevreuse_(genshin_impact)",
    "chiori_(genshin_impact)",
    "clorinde_(genshin_impact)",
    "emilie_(genshin_impact)",
    "escoffier_(genshin_impact)",
    "freminet_(genshin_impact)",
    "furina_(genshin_impact)",
    "lynette_(genshin_impact)",
    "lyney_(genshin_impact)",
    "navia_(genshin_impact)",
    "neuvillette_(genshin_impact)",
    "sigewinne_(genshin_impact)",
    "wriothesley_(genshin_impact)",

    # Natlan
    "chasca_(genshin_impact)",
    "citlali_(genshin_impact)",
    "iansan_(genshin_impact)",
    "kachina_(genshin_impact)",
    "kinich_(genshin_impact)",
    "mualani_(genshin_impact)",
    "olorun_(genshin_impact)",
    "xilonen_(genshin_impact)",

    # Snezhnaya / Fatui
    "columbina_(genshin_impact)",
    "dottore_(genshin_impact)",
    "tartaglia_(genshin_impact)",

    # Misc / crossover
    "aloy_(genshin_impact)",
    "aino_(genshin_impact)",
    "ineffa_(genshin_impact)",
    "citlali_(genshin_impact)",
    "linnea_(genshin_impact)",
    "nicole_(genshin_impact)",

    # ── Honkai: Star Rail ─────────────────────────────────────────────────────
    # 5-star female
    "acheron_(honkai:_star_rail)",
    "aglaea_(honkai:_star_rail)",
    "bailu_(honkai:_star_rail)",
    "black_swan_(honkai:_star_rail)",
    "bronya_(honkai:_star_rail)",
    "castorice_(honkai:_star_rail)",
    "cerydra_(honkai:_star_rail)",
    "cipher_(honkai:_star_rail)",
    "clara_(honkai:_star_rail)",
    "cyrene_(honkai:_star_rail)",
    "evanescia_(honkai:_star_rail)",
    "evernight_(honkai:_star_rail)",
    "feixiao_(honkai:_star_rail)",
    "firefly_(honkai:_star_rail)",
    "fu_xuan_(honkai:_star_rail)",
    "fugue_(honkai:_star_rail)",
    "himeko_(honkai:_star_rail)",
    "huohuo_(honkai:_star_rail)",
    "hyacine_(honkai:_star_rail)",
    "hysilens_(honkai:_star_rail)",
    "jade_(honkai:_star_rail)",
    "jingliu_(honkai:_star_rail)",
    "kafka_(honkai:_star_rail)",
    "lingsha_(honkai:_star_rail)",
    "rappa_(honkai:_star_rail)",
    "robin_(honkai:_star_rail)",
    "ruan_mei_(honkai:_star_rail)",
    "seele_(honkai:_star_rail)",
    "silver_wolf_(honkai:_star_rail)",
    "sparkle_(honkai:_star_rail)",
    "the_dahlia_(honkai:_star_rail)",
    "the_herta_(honkai:_star_rail)",
    "topaz_(honkai:_star_rail)",
    "tribbie_(honkai:_star_rail)",
    "yunli_(honkai:_star_rail)",

    # 4-star female
    "asta_(honkai:_star_rail)",
    "guinaifen_(honkai:_star_rail)",
    "hanya_(honkai:_star_rail)",
    "herta_(honkai:_star_rail)",
    "hook_(honkai:_star_rail)",
    "lynx_(honkai:_star_rail)",
    "march_7th_(honkai:_star_rail)",
    "natasha_(honkai:_star_rail)",
    "pela_(honkai:_star_rail)",
    "qingque_(honkai:_star_rail)",
    "serval_(honkai:_star_rail)",
    "sushang_(honkai:_star_rail)",
    "tingyun_(honkai:_star_rail)",
    "xueyi_(honkai:_star_rail)",
    "yukong_(honkai:_star_rail)",

    # ── Zenless Zone Zero ─────────────────────────────────────────────────────
    # Cunning Hares
    "anby_demara",
    "nicole_demara",
    "nekomiya_mana",

    # Victoria Housekeeping
    "corin_wickes",
    "ellen_joe",
    "rina_(zenless_zone_zero)",

    # Criminal Investigation Special Response Team
    "qingyi_(zenless_zone_zero)",
    "jane_doe_(zenless_zone_zero)",
    "zhu_yuan",

    # Sons of Calydon
    "burnice_(zenless_zone_zero)",
    "lucy_(zenless_zone_zero)",
    "piper_wheel",
    "caesar_(zenless_zone_zero)",
    "pulchra_(zenless_zone_zero)",

    # H.S.O. Section 6
    "hoshimi_miyabi",
    "soukaku_(zenless_zone_zero)",
    "yanagi_(zenless_zone_zero)",

    # Stars of Lyra / other factions
    "astra_yao",
    "evelyn_(zenless_zone_zero)",
    "grace_howard",
    "koleda_(zenless_zone_zero)",

    # Yunkui Summit
    "ju_fufu_(zenless_zone_zero)",
    "pan_yinhu_(zenless_zone_zero)",
    "ye_shunguang_(zenless_zone_zero)",
    "yixuan_(zenless_zone_zero)",

    # Spook Shack / newer factions
    "alice_(zenless_zone_zero)",
    "dialyn_(zenless_zone_zero)",
    "lucia_(zenless_zone_zero)",
    "yuzuha_(zenless_zone_zero)",

    # Angels of Delusion / Krampus Compliance Authority
    "aria_(zenless_zone_zero)",
    "banyue_(zenless_zone_zero)",
    "nangong_yu_(zenless_zone_zero)",
    "promeia_(zenless_zone_zero)",
    "sunna_(zenless_zone_zero)",

    # Obol Squad / misc
    "soldier_11_(zenless_zone_zero)",
    "vivian_(zenless_zone_zero)",
    "cissia_(zenless_zone_zero)",
]